...
//Number of particles. Some valid numbers are 100, 500, 2000, and 10000. Maximum
//is 10000. Doesn't convege with 1000.
...
void bench_read_data(FILE *position_f, FILE *laser_f, double position[3], double laser[181][2])
{
    int i;
    float px, py, pa, scan;
...
/*
 * Commented because of the test of the
 * store_particles() function
 *
 */
/*
   void calc_mean()
   {
   register int i;
   double mean_x=0, mean_y=0, stdev_x=0, stdev_y=0;
   int invalid=0;

   double mean_sin=0, mean_cos=0, mean_a;
   ...
   printf("\rMean x:%.2lf , y:%.2lf , a:%.2lf StdDev x:%.5lf, y:%.5lf", mean_x,  mean_y, mean_a*57.3,  stdev_x, stdev_y);
   }
/**/
...
void load_map(char *file_name)
{
    ...
    f = fopen("rth.map", "r");
    if (f != NULL) {
        fscanf(f, "mapd %i %i", &map.size_x, &map.size_y);
...
inline void do_action_model3(float odom_ang, float odom_tr, float xo, float yo, float to, float *xr, float *yr, float *tr, FILE *rand_f)
{
    ...
    odom_tr_b = odom_tr   + (odom_tr  * norm_t * gain_t) + (odom_ang * norm_r * gain_r * 0.5);
    odom_ang_b = odom_ang + (odom_ang * norm_r * gain_r) + (odom_tr  * norm_r * gain_r);
    ...
    while(count<200)
    {
        ...
        if ((int)(dif_x*1000) || (int)(dif_y*1000) || (int)(dif_a*1000))
        {
            ...
            for (i=0; i < MAX_PARTICLES; i++)
            {
                ...
                do_action_model3(odom_ang, odom_tr, particle[i].x, particle[i].y, particle[i].a, &particle[i].x, &particle[i].y, &particle[i].a, rand_f);
            ...
            if ((count % skip_obs)==0)
            {
                ...
                for (i=0; i < MAX_PARTICLES; i++)
                {
                    particle[i].w = do_observation_model(particle[i].x, particle[i].y, particle[i].a, laser);
