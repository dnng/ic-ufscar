...
void bench_read_data(FILE *position_f,
                     FILE *laser_f,
                     double position[3],
                     double laser[181][2])
{
...
void calc_mean()
{
    ...
    printf("\rMean x:%.2lf , y:%.2lf , a:%.2lf StdDev x:%.5lf, y:%.5lf", mean_x,
            mean_y, mean_a*57.3,  stdev_x, stdev_y);
}
...
inline void rand_position(float *rx, float *ry, float *ra, FILE *rand_f)
{
    ...
    *ra=((bench_rand(rand_f)%62)-31)/10.0;
    ...
    while (map.grid[(int)((*rx) * map.scale)]
                   [map.size_y- ((int)((*ry) * map.scale))]!=-1)
    {
        *rx = (bench_rand(rand_f) % map.size_x*100)/(map.scale*100);
        *ry = (bench_rand(rand_f) % map.size_y*100)/(map.scale*100);
    }
...
inline void resample_one(int i, FILE *rand_f)
{
    ...
    while (map.grid[(int) (particle[ref].x*map.scale)]
                   [(int) (map.size_y-(particle[ref].y*map.scale))] != -1)
    {
        ref=bench_rand(rand_f)%MAX_PARTICLES;
    }
...
inline unsigned do_observation_model(float x,
                                     float y,
                                     float a,
                                     double laser[181][2])
{
...
inline void do_action_model3(float odom_ang,
                             float odom_tr,
                             float xo,
                             float yo,
                             float to,
                             ...
                             FILE *rand_f)
{
    ...
    odom_tr_b = odom_tr +
        (odom_tr  * norm_t * gain_t) + (odom_ang * norm_r * gain_r * 0.5);
    odom_ang_b = odom_ang +
        (odom_ang * norm_r * gain_r) + (odom_tr  * norm_r * gain_r);
    ...
    while(count<200)
    {
        ...
        if ((int)(dif_x*1000) || (int)(dif_y*1000) || (int)(dif_a*1000))
        {
            ...
            for (i=0; i < MAX_PARTICLES; i++)
            {
                ...
                do_action_model3(odom_ang, odom_tr,
                                 particle[i].x,
                                 particle[i].y,
                                 particle[i].a,
                                 &particle[i].x,
                ...
                if (map.grid[(int) (particle[i].x*map.scale)]
                            [(int) (map.size_y-(particle[i].y*map.scale))]
                            != -1)
                {
